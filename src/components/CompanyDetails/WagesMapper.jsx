...
const WagesMapper = ({ companyId, selectedTab }) => {
    const [editMode, setEditMode] = useState(false);
    const [newRows, setNewRows] = useState([]);
    const [openModal, setOpenModal] = useState(false);

    const { wagesMapperData, fetchWagesMapperAttributes, postWagesMapperAttributes } = useContext(WagesContext);
    const addWagesFormik = useFormik({ initialValues: wagesMapperData });

    const tableData = {
        columns: ["Sheet ID", "Employee", "Employee Wages", "Start Date", "End Date"],
        rows: [{
    ...
    useEffect(() => {
        fetchWagesMapperAttributes(companyId);
    ...
    const handleSaveChanges = () => {
        ...
        postWagesMapperAttributes(companyId, { ...addWagesFormik.values, rows: updatedRows });
        setEditMode(false);
        setNewRows([]);
    };
                    ...
                    <Container>
                        <TableContainer sx={styles.tableContainer}>
                            <Table>
                                ...
                                <WagesMapperTableBody filledRows={[...tableData.rows, ...newRows]} editMode={editMode} addWagesFormik={addWagesFormik} wagesMapperData={wagesMapperData} />
                            </Table>
                        </TableContainer>
                    </Container>
                </form>

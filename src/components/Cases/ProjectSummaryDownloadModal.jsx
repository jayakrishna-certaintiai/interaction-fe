...
const ProjectSummaryDownloadModal = ({ open, usedfor, handleClose, updatePurpose, projects, postUpdate }) => {
    ...
    const handleSelectSummary = (projectId, checked) => {
        if (checked) {
            setSelectedSummary([...selectedSummary, projectId]);
            setSummaryError("");
        } else {
            const updateSummary = selectedSummary.filter(pr => pr !== projectId);
            setSelectedSummary(updateSummary);
        }
    }
    const getFilteredSummary = (filteredSummary) => {
        setFilteredSummary(filteredSummary);
    }
    const handleSelectAllSummary = (checked) => {
        if (checked) {
            const allSummary = filteredSummary.map(pr => (updatePurpose === "Surveys" ? pr?.caseProjectId : updatePurpose === "Summary" ? pr.TechnicalSummaryId : ""))
            setSelectedSummary(allSummary);
            setSummaryError("");
        } else {
            setSelectedSummary([]);
        }
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${BaseURL}/api/v1/projects/download-technicalSummary-report`,
                {
                    headers: {
                        Authorization: `Bearer ${token_obj?.accessToken}`,
                    },
                    params: {
                        technicalSummaryIds: JSON.stringify(selectedSummary),
            ...
            if (response.data) {
                ...
                let fileName = "technical-summary-report.zip";
            ...
            <Paper sx={styles.paperStyle}>
                <Box sx={{ ...styles.box1Style, py: 1 }}>
                    <Box sx={{ display: "flex", flexDirection: "column", py: 1, width: "100%", alignItems: "left", justifyContent: "flex-start", mb: -2, }}>
                        <Typography variant="h6" sx={{ ...styles.titleStyle, fontSize: "15px", mb: 0, borderBottom: "1px solid #E4E4E4", width: "100%", }}>Download Summary</Typography>
                    </Box>
                    ...
                    <form>
                        <Box sx={{ height: "90%" }}>
                            <ProjectDownDataModal projects={projects} handleSelectSummary={handleSelectSummary} handleSelectAllSummary={handleSelectAllSummary} updatePurpose={updatePurpose} getFilteredSummary={getFilteredSummary} documentType={documentType} />
                            {summaryError && (
                                <Typography color="error" sx={{ mt: 1 }}>{summaryError}</Typography>
                            )}
                        </Box>
                        <Box style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingLeft: "1rem" }}>
                            <Box><Typography>Selected {selectedSummary?.length || 0} {updatePurpose} out of {filteredSummary?.length || 0}</Typography></Box>
...
export default ProjectSummaryDownloadModal
